...
(function () {
  ...
  const 相簿 = (function () {
    ...
    function 開() {
      return new Promise((res) => {
        if (db) return res(db);
        const r = indexedDB.open("男生女生~耶！照片", 1);
  ...
  const 點名 = (function () {
    ...
    async function 匯出() {
      ...
      const 資料 = { 格式: "男生女生~耶！點名名單", 版本: 1, 匯出時間: new Date().toISOString(), 學生: [] };
      for (const s of 名單) {
    ...
    async function 通關() {
      ...
      $("#重點內容").innerHTML = `
        <div class="通關卡">
          ...
          <p class="通關說明">${全對 ? `${本輪對錯.length}題都一次答對，很厲害！` : "全部答對了，再複習一次會更熟喔。"}</p>
          <div class="按鈕列">
            <button class="次要鈕" id="鈕回重點">回重點清單</button>
...
/* ── 畫面上的數量文字一律從 data.js 填，不要寫死（坑 16 的根治版）─────────
   範本原本把「四個重點」「12 題」「20 張圖卡」寫死在 index.html，
   換單元時**不會報錯、功能測試也全過**，只能靠人眼掃過每一段說明文字。
   線對稱圖形（7 重點 21 題）就是在交付前的驗收才抓到「開始評量（12 題）」。
   → 改成開場時自動填；index.html 那幾個 span 是佔位值，看到不用改。 */
(function 填數量文字() {
